...
ADD COLUMN IF NOT EXISTS preferred_physician uuid,
ADD COLUMN IF NOT EXISTS emergency_contact jsonb,
ADD COLUMN IF NOT EXISTS family_history jsonb;

-- Add constraints separately to avoid IF NOT EXISTS issues
DO $$ 
BEGIN
    -- Add marital status constraint if it doesn't exist
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.check_constraints 
        WHERE constraint_name = 'patients_marital_status_check'
    ) THEN
    ...
    -- Add occupation type constraint if it doesn't exist
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.check_constraints 
        WHERE constraint_name = 'patients_occupation_type_check'
    ) THEN
        ALTER TABLE patients ADD CONSTRAINT patients_occupation_type_check 
        CHECK (occupation_type IN ('sedentary', 'moderate', 'heavy'));
    END IF;
    ...
    -- Handle duplicate uhid values before adding unique constraint
    -- Update duplicate uhid values to be unique
    UPDATE patients 
    SET uhid = uhid || '_' || id::text 
    WHERE uhid IN (
        SELECT uhid 
        FROM patients 
    ...
    -- Add unique constraint for uhid if it doesn't exist
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE constraint_name = 'patients_uhid_key'
    ) THEN
        ALTER TABLE patients ADD CONSTRAINT patients_uhid_key UNIQUE (uhid);
    END IF;

    -- Add foreign key for preferred_physician if it doesn't exist
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE constraint_name = 'patients_preferred_physician_fkey'
    ) THEN
        ALTER TABLE patients ADD CONSTRAINT patients_preferred_physician_fkey 
        FOREIGN KEY (preferred_physician) REFERENCES auth.users(id);
    END IF;
END $$;
...
-- Add unique constraint for icd11_code
DO $$ 
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE constraint_name = 'disease_mappings_icd11_code_key'
    ) THEN
        ALTER TABLE disease_mappings ADD CONSTRAINT disease_mappings_icd11_code_key UNIQUE (icd11_code);
    END IF;
END $$;
...
-- Add constraints for ayurvedic_herbs
DO $$ 
BEGIN
    -- Add primary_dosha_effect constraint if it doesn't exist
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.check_constraints 
        WHERE constraint_name = 'ayurvedic_herbs_primary_dosha_effect_check'
    ) THEN
        ALTER TABLE ayurvedic_herbs ADD CONSTRAINT ayurvedic_herbs_primary_dosha_effect_check 
        CHECK (primary_dosha_effect IN ('vata', 'pitta', 'kapha', 'vata-pitta', 'pitta-kapha', 'vata-kapha', 'tridoshic'));
    END IF;

    -- Add virya constraint if it doesn't exist
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.check_constraints 
        WHERE constraint_name = 'ayurvedic_herbs_virya_check'
    ) THEN
        ALTER TABLE ayurvedic_herbs ADD CONSTRAINT ayurvedic_herbs_virya_check 
        CHECK (virya IN ('ushna', 'shita', 'variable'));
    END IF;
END $$;
...
DROP POLICY IF EXISTS "Users can read disease mappings" ON disease_mappings;
...
DROP POLICY IF EXISTS "Users can read herbs" ON ayurvedic_herbs;
...
INSERT INTO disease_mappings (
  ...
  dosha_involvement, primary_dosha, classical_references, severity_classification, modern_lab_correlations
) VALUES
-- Gastrointestinal
(
  '8A62.0',
...
USING gin(to_tsvector('english', sanskrit_name || ' ' || hindi_name || ' ' || malayalam_name || ' ' || common_name));

CREATE INDEX IF NOT EXISTS idx_disease_mappings_fts ON disease_mappings 
USING gin(to_tsvector('english', icd11_name || ' ' || ayurvedic_name));
